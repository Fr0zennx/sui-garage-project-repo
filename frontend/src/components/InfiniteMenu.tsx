...
class ArcballControl {
  ...
  constructor(canvas: HTMLCanvasElement, updateCallback: (deltaTime: number) => void, itemCount: number) {
    this.canvas = canvas;
    ...
    // Keyboard control for item navigation
    window.addEventListener('keydown', (e: KeyboardEvent) => {
      if (e.key === 'ArrowRight') {
        e.preventDefault();
        this.currentItemIndex = (this.currentItemIndex + 1) % this.itemCount;
        this.rotateToItem(this.currentItemIndex);
      } else if (e.key === 'ArrowLeft') {
        e.preventDefault();
        this.currentItemIndex = (this.currentItemIndex - 1 + this.itemCount) % this.itemCount;
        this.rotateToItem(this.currentItemIndex);
      }
    });
  ...
  rotateToItem(index: number) {
    // Calculate rotation needed to bring item to front
    const anglePerItem = (Math.PI * 2) / this.itemCount;
    const targetAngle = -index * anglePerItem;
    const axis = vec3.fromValues(0, 1, 0);
    quat.setAxisAngle(this.targetOrientation, axis, targetAngle);
  ...
  update(deltaTime: number, targetFrameDuration = 16) {
    ...
    if (this.isAnimating) {
      const lerpFactor = 0.15; // Animation speed (0-1, lower = smoother/slower)
      quat.slerp(this.orientation, this.orientation, this.targetOrientation, lerpFactor);
      quat.normalize(this.orientation, this.orientation);

      // Check if animation is complete
      const diff = quat.create();
      quat.multiply(diff, quat.conjugate(quat.create(), this.orientation), this.targetOrientation);
      const angle = 2 * Math.acos(Math.min(1, Math.abs(diff[3])));
      
      if (angle < 0.01) {
        quat.copy(this.orientation, this.targetOrientation);
        this.isAnimating = false;
        this._rotationVelocity = 0;
      } else {
        this._rotationVelocity = angle * 0.5;
      }
    } else {
      this._rotationVelocity *= 0.9;
    }

    this.rotationVelocity = this._rotationVelocity;
...
class InfiniteGridMenu {
  ...
  init(onInit: ((menu: InfiniteGridMenu) => void) | null) {
    ...
    // Create circular arrangement for items instead of icosahedron
    this.DISC_INSTANCE_COUNT = this.items.length;
    this.instancePositions = [];
    const angleStep = (Math.PI * 2) / this.items.length;
    for (let i = 0; i < this.items.length; i++) {
      const angle = i * angleStep;
      const x = Math.sin(angle) * this.SPHERE_RADIUS;
      const y = 0;
      const z = -Math.cos(angle) * this.SPHERE_RADIUS;
    ...
    this.control = new ArcballControl(this.canvas, deltaTime => this.onControlUpdate(deltaTime), this.items.length);
  ...
  initTexture() {
    ...
    // Create numbered images for each item
    this.items.forEach((item, i) => {
      ...
      // Add number at bottom center
      ctx.fillStyle = '#2a7ab5';
  ...
  animate(deltaTime: number) {
    ...
    let positions = this.instancePositions.map((p: vec3) =>
      vec3.transformQuat(vec3.create(), p, this.control.orientation)
    );
    const scale = 0.35;
    const SCALE_INTENSITY = 0.6;
    positions.forEach((p: vec3, ndx: number) => {
  ...
  onControlUpdate(deltaTime: number) {
    ...
    // Update active item based on current rotation
    const itemIndex = this.control.currentItemIndex % Math.max(1, this.items.length);
    this.onActiveItemChange(itemIndex);
    
    const isMoving = this.control.isAnimating;
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: InfiniteMenuProps) {
  ...
  return (
    <div className="infinite-menu-wrapper">
      <canvas className="infinite-menu-canvas" ref={canvasRef} />
